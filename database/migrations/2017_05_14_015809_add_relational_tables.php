...
class AddRelationalTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::table('request', function($table) {
            ...
            $table->foreign('id_user_req')->references('id_user')->on('users');
            $table->integer('id_garbage')->unsigned();
            $table->foreign('id_garbage')->references('id_garbage')->on('garbage');
        });

        Schema::table('request_assignment', function($table) {
            $table->integer('id_req')->unsigned();
            $table->foreign('id_req')->references('id_req')->on('request');
            $table->integer('id_user_assign')->unsigned();
            $table->foreign('id_user_assign')->references('id_user')->on('users');
        });
